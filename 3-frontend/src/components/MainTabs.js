...
import {
  Box, Flex, Grid, VStack, HStack, Button, Icon, Heading, Tag,
...
const MainPage = () => {
  ...
  // Memoize the heavy panes so MainPage re-renders don't recreate trees
  const RobotPane = useMemo(() => (
    <Box
      position="absolute" top={0} left={0} width="100%" height="100%"
      display={view === "robot" ? "block" : "none"}
    >
      <Grid templateColumns={{ base: "1fr", md: "2fr 1fr" }} gap={2} h="100%" overflow="hidden">
  ...
  return (
    <Flex direction="column" h="100vh" overflow="hidden" bg={appBg}>
      {/* Header */}
      <Flex as="header" align="center" justify="space-between" px={6} py={3} bg={headerBg} shadow="sm" zIndex={1}>
        <Heading size="md">6AR Robot Controller Software V1.0.0</Heading>

        <HStack spacing={2}>
          <ButtonGroup isAttached bg={headerBg} px={4} py={1} shadow="sm">
            <Button variant={view === "robot" ? "solid" : "outline"} onClick={() => setView("robot")}>Robot</Button>
            <Button variant={view === "program" ? "solid" : "outline"} onClick={() => setView("program")}>Program</Button>
            <Button variant={view === "run" ? "solid" : "outline"} onClick={() => setView("run")}>Run</Button>
          </ButtonGroup>

          <Tag size="sm" colorScheme={connected ? "green" : "red"}>{connected ? "Online" : "Offline"}</Tag>
          ...
          <Button size="sm" leftIcon={<Icon as={PiSlidersHorizontalBold} />} onClick={onOpen}>Settings</Button>
          <Button size="sm" colorScheme="red" leftIcon={<Icon as={PiWarningCircleBold} />} onClick={stopAll}>E-Stop</Button>
        </HStack>
      </Flex>
