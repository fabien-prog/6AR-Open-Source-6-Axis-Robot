...
// 1) Accent colors for each block type (stable)
const blockColors = {
  "Move L": "primary.500",
  "Move J": "primary.500",
  Home: "red.500",
...
// 2) Compute nesting levels for indentation (pure)
...
// Render a badge-based summary (pure)
function renderSummary(block, variables = []) {
  switch (block.type) {
    ...
    case "Move J": {
      ...
      return (
        <HStack spacing={3} wrap="wrap">
          ...
          <Badge display="inline-flex" alignItems="center" colorScheme="blue" px={3} py={2} borderRadius="md" fontSize="sm">
            <Icon boxSize='16px' as={PiTarget} mr={2} /> {pt}
          </Badge>
          <Badge display="inline-flex" alignItems="center" colorScheme="green" px={3} py={2} borderRadius="md" fontSize="sm">
            <Icon boxSize='16px' as={PiSpeedometer} mr={2} /> {block.speed} {block.type === "Move L" ? "mm/s" : "°/s"}
          </Badge>
          <Badge display="inline-flex" alignItems="center" colorScheme="orange" px={3} py={2} borderRadius="md" fontSize="sm">
            <Icon boxSize='16px' as={PiMapPinAreaBold} mr={2} /> {block.referenceType === "WObj" ? (block.referenceObject || "–") : "World"}
          </Badge>
        </HStack>
      );
    }
    case "Home":
      return (
        <Badge display="inline-flex" alignItems="center" colorScheme="red" px={3} py={2} borderRadius="md" fontSize="sm">
    ...
    case "SetDO":
      return (
        <HStack spacing={3} wrap="wrap">
          <Badge display="inline-flex" alignItems="center" colorScheme="blue" px={3} py={2} borderRadius="md" fontSize="sm">
            Set Digital Output {block.pin} to :
          </Badge>
          <Badge display="inline-flex" alignItems="center" colorScheme={block.state === "1" ? "green" : "red"} px={3} py={2} borderRadius="md" fontSize="sm">
            {block.state}
          </Badge>
        </HStack>
      );
    case "WaitDI":
      return (
        <HStack spacing={3} wrap="wrap">
          <Badge display="inline-flex" alignItems="center" colorScheme="blue" px={3} py={2} borderRadius="md" fontSize="sm">
            Wait for Digital Input {block.pin} to be :
          </Badge>
          <Badge display="inline-flex" alignItems="center" colorScheme={block.state === "1" ? "green" : "red"} px={3} py={2} borderRadius="md" fontSize="sm">
            {block.state}
          </Badge>
    ...
    case "If": {
      const left = block.variableSource === "IO" ? block.io
        : block.variableSource === "Variable" ? block.io
          : block.condition;
      return (
        <HStack spacing={3} wrap="wrap">
          <Badge display="inline-flex" alignItems="center" colorScheme="cyan" px={3} py={2} borderRadius="md" fontSize="sm">
    ...
    case "For Loop":
      return (
        <HStack spacing={3} wrap="wrap">
          <Badge display="inline-flex" alignItems="center" colorScheme="pink" px={3} py={2} borderRadius="md" fontSize="sm">
    ...
    case "Counter":
      return (
        <Badge display="inline-flex" alignItems="center" colorScheme="teal" px={3} py={2} borderRadius="md" fontSize="sm">
    ...
    case "Console Log":
      return (
        <Badge display="inline-flex" alignItems="center" colorScheme="purple" px={3} py={2} borderRadius="md" fontSize="sm">
          <Icon boxSize='16px' as={PiTerminal} mr={2} /> console.{block.level}("{block.message}")
        </Badge>
      );
    case "Math": {
      // lightweight preview; errors just show "—"
      let preview = "—";
      try {
...
function BlockEditor({ state, dispatch, filter }) {
  const { digitalInputs, digitalOutputs } = useData();

  // memoize indent levels
  const indentLevels = useMemo(() => computeIndentLevels(state.blocks), [state.blocks]);

  // memoize visible filtering
  const visible = useMemo(() => {
    const f = (filter || "").toLowerCase();
    return state.blocks
      .map((blk, i) => ({ blk, i }))
      .filter(({ blk }) =>
        blk.type.toLowerCase().includes(f) ||
        (blk.comment || "").toLowerCase().includes(f)
      );
  }, [state.blocks, filter]);
  ...
  // Dispatch helpers (stable)
  const updateBlock = useCallback((i, field, val) =>
    dispatch({ type: "UPDATE_BLOCK", index: i, payload: { [field]: val } }), [dispatch]);
  const removeBlock = useCallback((i) =>
    dispatch({ type: "REMOVE_BLOCK", index: i }), [dispatch]);
  const duplicateBlock = useCallback((i) => {
  ...
  // Render block parameters with compact spacing. (Callbacks kept stable)
  const renderBlockParams = useCallback((block, index) => {
    switch (block.type) {
      case "Move L":
      case "Move J": {
        ...
        return (
          <>
            ...
            {/* Cartesian entry for Move L, or Move J in cartesian mode */}
            {block.src === "manual" && (((isMoveL) || block.moveMode === "cartesian")) && (
              <HStack spacing={2} mb={2}>
                <Text w="100px" fontSize="sm">Cartesian:</Text>
                <Input
                  size="sm"
            ...
            {/* Joint entry for Move J in joint mode */}
            ...
            {/* Speed */}
            <HStack spacing={2} mb={2}>
              <Text w="100px" fontSize="sm">Speed ({isMoveL ? "mm/s" : "°/s"}):</Text>
              <NumberInput
                size="sm"
                value={block.speed || ""}
      ...
      case "Home":
        return <Text fontSize="xs" color="gray.300">No parameters required.</Text>;

      case "SetDO":
        return (
          <>
            <HStack mb={2}>
              <Text w="100px" fontSize="sm">Pin:</Text>
              <Select size="sm" value={block.pin} onChange={e => updateBlock(index, "pin", e.target.value)}>
                {digitalOutputs.map(o => (<option key={o.id} value={o.id}>DO_{o.id}</option>))}
              </Select>
            </HStack>
            <HStack mb={2}>
              <Text w="100px" fontSize="sm">State:</Text>
              <Select size="sm" value={block.state} onChange={e => updateBlock(index, "state", e.target.value)}>
        ...
        return (
          <>
            <HStack mb={2}>
              <Text w="100px" fontSize="sm">Pin:</Text>
              <Select size="sm" value={block.pin} onChange={e => updateBlock(index, "pin", e.target.value)}>
                {digitalInputs.map(i => (<option key={i.id} value={i.id}>DI_{i.id}</option>))}
              </Select>
            </HStack>
            <HStack mb={2}>
              <Text w="100px" fontSize="sm">State:</Text>
        ...
        return (
          <>
            <HStack spacing={2} mb={1}>
              ...
              <Select size="sm" value={source} onChange={e => updateBlock(index, "variableSource", e.target.value)}>
              ...
              {source === "IO" && (
                <Select size="sm" value={block.io || ""} onChange={e => updateBlock(index, "io", e.target.value)}>
              ...
              {source === "Variable" && (
                <Select size="sm" value={block.io || ""} onChange={e => updateBlock(index, "io", e.target.value)}>
                  ...
                  {state.variables.map((v, i) => (<option key={i} value={v.name}>{v.name}</option>))}
                </Select>
              )}
              {source === "Constant" && (
                <Input size="sm" placeholder="Constant" value={block.condition || ""} onChange={e => updateBlock(index, "condition", e.target.value)} />
              )}
            </HStack>

            <HStack spacing={2} mb={1}>
              <Text w="70px" fontSize="sm">Operator:</Text>
              <Select size="sm" value={block.operator || "=="} onChange={e => updateBlock(index, "operator", e.target.value)}>
            ...
            <HStack spacing={2} mb={1}>
              ...
              <Input size="sm" placeholder="Value" value={block.value || ""} onChange={e => updateBlock(index, "value", e.target.value)} />
            </HStack>
          </>
        );
      }
        ...
        return (
          <>
            <HStack spacing={2} mb={1}>
              ...
              <Select size="sm" placeholder="Select variable" value={block.counter} onChange={e => updateBlock(index, "counter", e.target.value)}>
                {numberVars.map((v, i) => (<option key={i} value={v.name}>{v.name}</option>))}
              </Select>
            </HStack>

            <HStack spacing={2} mb={1}>
              <Text w="70px" fontSize="sm">End Src:</Text>
              <Select size="sm" value={endSource} onChange={e => updateBlock(index, "endSource", e.target.value)}>
            ...
            {endSource === "Literal" ? (
              <HStack spacing={2} mb={1}>
                ...
                <Input size="sm" type="number" placeholder="e.g. 10" value={block.end} onChange={e => updateBlock(index, "end", e.target.value)} />
              </HStack>
            ) : (
              <HStack spacing={2} mb={1}>
                <Text w="70px" fontSize="sm">End Var:</Text>
                <Select size="sm" placeholder="Select variable" value={block.end} onChange={e => updateBlock(index, "end", e.target.value)}>
                  {numberVars.map((v, i) => (<option key={i} value={v.name}>{v.name}</option>))}
                </Select>
              </HStack>
            )}
            ...
            <HStack spacing={2} mb={1}>
              ...
              <Input size="sm" type="number" value={block.start} onChange={e => updateBlock(index, "start", e.target.value)} />
            </HStack>
            <HStack spacing={2} mb={1}>
              <Text w="70px" fontSize="sm">Step:</Text>
              <Input size="sm" type="number" value={block.step} onChange={e => updateBlock(index, "step", e.target.value)} />
            </HStack>
          </>
        );
      }
      ...
      case "Else":
      case "End For":
        return <Text fontSize="xs" color="gray.600">No parameters required.</Text>;
  ...
  return (
    <Droppable droppableId="blocks" direction="vertical" isDropDisabled={false} isCombineEnabled={false} ignoreContainerClipping={false}>
      {(provided) => (
        <VStack
                  ...
                  >
                    <HStack justify="space-between" align="start">
                      <VStack align="start" spacing={1}>
                        <Text fontWeight="bold" fontSize="xl">{block.type}</Text>
                        {renderSummary(block, state.variables)}
                      </VStack>
                      <HStack spacing={1}>
                        <Tooltip label="Move Up">
                          <Button size="sm" onClick={() => moveBlock(origIdx, origIdx - 1)}>
                            <PiArrowUp />
                          </Button>
                        </Tooltip>
                        ...
                        <Tooltip label="Duplicate">
                          <Button size="sm" onClick={() => duplicateBlock(origIdx)}>
                            <PiCopy />
                          </Button>
                        </Tooltip>
                          ...
                          <PopoverContent>
                            ...
                            <PopoverBody>
                              ...
                              <Input
                                ...
                                value={block.comment || ""}
                                placeholder="Comment…"
                                onChange={(e) => updateBlock(origIdx, "comment", e.target.value)}
                              />
                            </PopoverBody>
                          </PopoverContent>
                        </Popover>
                        <Tooltip label="Delete">
                          <Button size="sm" variant="outline" colorScheme="red" onClick={() => removeBlock(origIdx)}>
                            <PiTrash />
                          </Button>
                        </Tooltip>
                      </HStack>
