// src/lib/syncMotion.ts
export type JointStatus = { joint: number; position: number; velocity: number; acceleration: number; target: number | null };

export type MoveMultipleFn = (joints: number[], targets: number[], speeds: number[], accels: number[]) => void;

export type SonnerToastLike = {
  info?: (msg: string, opts?: { description?: string }) => void;
  success?: (msg: string, opts?: { description?: string }) => void;
  warning?: (msg: string, opts?: { description?: string }) => void;
  error?: (msg: string, opts?: { description?: string }) => void;
  message?: (msg: string, opts?: { description?: string }) => void;
};

export async function movePhysicalToVirtual(opts: {
  getAllJointStatus: () => Promise<JointStatus[] | null>;
  poseJoints: number[]; // target joint angles [j1…j6]
  parameters: Record<string, number | string | boolean>;
  moveMultiple: MoveMultipleFn;
  toast?: SonnerToastLike;
}) {
  const { getAllJointStatus, poseJoints, parameters, moveMultiple } = opts;
  const toast = opts.toast ?? {};

  const tErr = (title: string, description?: string) => toast.error?.(title, { description });
  const tInfo = (title: string, description?: string) => toast.info?.(title, { description });
  const tOk = (title: string, description?: string) => toast.success?.(title, { description });

  // ── 0) sanity: need 6 targets
  if (!Array.isArray(poseJoints) || poseJoints.length < 6) {
    tErr("Invalid targets", "Need 6 joint angles.");
    return;
  }

  // ── 1) Query current positions
  let statuses: JointStatus[] | null;
  try {
    statuses = await getAllJointStatus();
  } catch {
    tErr("Failed to query joint status");
    return;
  }
  if (!Array.isArray(statuses) || statuses.length !== 6) {
    tErr("Invalid joint status response");
    return;
  }

  // ── 2) Pre-allocate arrays
  const jointsIdx = [1, 2, 3, 4, 5, 6];
  const initial = new Array<number>(6);
  const final = new Array<number>(6);
  const deltas = new Array<number>(6);
  const vBase = new Array<number>(6);
  const aBase = new Array<number>(6);

  for (let i = 0; i < 6; i++) {
    initial[i] = Number(statuses[i]?.position) || 0;
    final[i] = Number(poseJoints[i]) || 0;
    deltas[i] = Math.abs(final[i] - initial[i]);

    const vmax = Number(parameters[`joint${i + 1}.maxSpeed`]) || 0;
    const amax = Number(parameters[`joint${i + 1}.maxAccel`]) || 0;

    // conservative: /3 like your original
    vBase[i] = Math.max(0, vmax / 3);
    aBase[i] = Math.max(0, amax / 3);
  }

  // ── 3) Early exit if every delta is tiny
  const EPS = 1e-3;
  let maxDelta = 0;
  for (let i = 0; i < 6; i++) if (deltas[i] > maxDelta) maxDelta = deltas[i];

  if (maxDelta <= EPS) {
    tInfo("Already at target");
    return;
  }

  // ── 4) helper: time under trapezoidal profile
  const trapezoidalTime = (delta: number, vmax: number, amax: number) => {
    if (amax <= 0 || vmax <= 0) return Infinity;

    const tA = vmax / amax;
    const xA = 0.5 * amax * tA * tA;

    if (delta < 2 * xA) {
      // triangular
      return 2 * Math.sqrt(Math.max(delta, 0) / amax);
    }

    // trapezoidal
    const xC = delta - 2 * xA;
    return 2 * tA + xC / vmax;
  };

  // ── 5) compute syncTime (slowest axis)
  let syncTime = 0.01;
  for (let i = 0; i < 6; i++) {
    const t = trapezoidalTime(deltas[i], vBase[i], aBase[i]);
    if (t > syncTime) syncTime = t;
  }

  if (!Number.isFinite(syncTime) || syncTime <= 0) {
    tErr("Invalid kinematics parameters", "Check max speed/accel limits.");
    return;
  }

  // ── 6) Solve per-axis vmax,aSync to fit exactly into syncTime
  const speeds = new Array<number>(6);
  const accels = new Array<number>(6);
  const tAmax = syncTime / 2;

  for (let i = 0; i < 6; i++) {
    const d = deltas[i];
    const amax = aBase[i];
    const vcap = vBase[i];

    let vmax: number;
    if (amax <= 0) {
      vmax = 0;
    } else {
      const xAmax = 0.5 * amax * tAmax * tAmax;

      if (d < 2 * xAmax) {
        // triangular-case peak
        vmax = Math.sqrt(Math.max(d * amax, 0));
      } else {
        // trapezoidal-case
        const disc = amax * amax * syncTime * syncTime - 4 * amax * d;
        vmax = disc < 0 ? amax * tAmax : (amax * syncTime - Math.sqrt(disc)) / 2;
      }
    }

    // cap by base speed and keep non-negative
    vmax = Math.max(0, Math.min(vmax || 0, vcap));

    // aSync derived from tAmax (avoid div0)
    const aSync = tAmax > 0 ? vmax / tAmax : 0;

    speeds[i] = vmax;
    accels[i] = aSync;
  }

  // ── 7) round & clamp (avoid zeros to keep firmware happy)
  const round4 = (v: number) => Math.round(v * 10000) / 10000;
  const MIN_V = 0.1;
  const MIN_A = 0.1;

  for (let i = 0; i < 6; i++) {
    final[i] = round4(final[i]);
    speeds[i] = round4(Math.max(MIN_V, speeds[i] || 0));
    accels[i] = round4(Math.max(MIN_A, accels[i] || 0));
  }

  // ── 8) Emit single MoveMultiple
  moveMultiple(jointsIdx, final, speeds, accels);
  tOk("MoveMultiple sent", `syncTime ≈ ${round4(syncTime)}s`);
}
